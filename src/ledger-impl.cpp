...
LedgerImpl::onNewRecordNotification(const Interest& interest)
{
	...
	m_scheduler.schedule(time::milliseconds(dist(eng)), [&, recordName] {
		...
		m_network.expressInterest(recordInterest,
															bind(&LedgerImpl::onRequestedData, this, _1, _2, -1, true),
...
void
LedgerImpl::onRequestedData(const Interest& interest, const Data& data, int vectIndex, bool isFirst)
{
	...
	std::cout << (producedBy) << "produced by \n";
	std::time_t present = std::time(0);
	...
	*/
	...
	if(isFirst == true){
		...
		dledger::Record dataRecord = dledger::Record(data);
		std::vector<Record> newWaitingList;
		Name rootName = Name(dataRecord.getRecordName());
		newWaitingList.push_back(dataRecord);
		m_waitingLists.push_back(newWaitingList);
		int vectPlace = m_waitingLists.size();

		std::list<Name> ancestors = dataRecord.getPointersFromHeader();

		//Start fetching the ancestor records and putting them into the same list. Then eventually schedule a deletion.
		//Okay, so we've added the record name to the list... now we have to go ahead and get the ancestors. 
		std::list <Name>::iterator it; 
		//for(it = g.begin(); it != g.end(); ++it) 
		for(it = ancestors.begin(); it != ancestors.end(); it++){
			Interest dataRequest(*it);
			dataRequest.setCanBePrefix(false);
			dataRequest.setMustBeFresh(true);
			m_network.expressInterest(dataRequest,
														bind(&LedgerImpl::onRequestedData, this, _1, _2, vectPlace, false),
														bind(&LedgerImpl::onNack, this, _1, _2),
														bind(&LedgerImpl::onTimeout, this, _1));

			}
		} else {
			//Okay, so we we got a response from the first interest. 
			if (!checkValidityOfRecord(data)) {
				std::cout << "Requested ancestor is not valid \n";
				//Remove the whole waiting list if it's not valid
				//Alternative: just remove the one that called this? If so, must add to args (element that called)
				m_waitingLists.erase(m_waitingLists.begin() + vectIndex);				
				return;
			}
			//We do the same thing as above, except we don't make a new list and we do check if the record is already in the db
				dledger::Record dataRecord = dledger::Record(data);
				//Check if record is in the database:
				if (m_backend.getRecord(Record(data).getRecordName())){
					std::cout << "reverse check ";
					//If it's in the database, trigger a reverse check through the vector: for each element, check if the record's 
					//Ancestors are in the backend/db. If they are, add that record to the db. 
					std::vector<Record>::reverse_iterator rit = m_waitingLists[vectIndex].rbegin();
					for (; rit != m_waitingLists[vectIndex].rend(); ++rit){
						Record currRec = *rit;
						std::list<Name> ancestors = currRec.getPointersFromHeader();
						std::list <Name>::iterator it; 
						//for(it = g.begin(); it != g.end(); ++it) 
						bool check = true;
						for(it = ancestors.begin(); it != ancestors.end(); it++){
							Name nameToCheck = *it;
							if(!(m_backend.getRecord(nameToCheck))){
								check = false;
							}
						}
						if (check == true){
							m_backend.putRecord(std::make_shared<Data>(currRec.getRecordName()));
						}
					}

					return;
				} else{
					std::cout << "Getting more ancestors";
					//If it's not, get the ancestors
					std::list<Name> ancestors = dataRecord.getPointersFromHeader();
					std::list <Name>::iterator it; 
					for(it = ancestors.begin(); it != ancestors.end(); it++){
						Interest dataRequest(*it);
						...
						dataRequest.setMustBeFresh(true);
						m_network.expressInterest(dataRequest,
																	bind(&LedgerImpl::onRequestedData, this, _1, _2, vectIndex, false),
																	bind(&LedgerImpl::onNack, this, _1, _2),
																	bind(&LedgerImpl::onTimeout, this, _1));
					}
					//Finally, go ahead and check if everything in the vectIndex is 
...
LedgerImpl::onRecordRequest(const Interest& interest)
{
	...
	auto desiredData = m_backend.getRecord(interest.getName());
	if (desiredData) {
