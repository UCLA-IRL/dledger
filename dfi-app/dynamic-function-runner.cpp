...
wasm_module_t* DynamicFunctionRunner::compile(const ndn::Block& block) {
...
wasm_instance_t *DynamicFunctionRunner::instantiate(wasm_module_t *module, const wasm_extern_t **imports, size_t import_length) {
    // Instantiate.
...
wasmtime_linker_t *DynamicFunctionRunner::instantiate_wasi(wasm_module_t *module){
    // Instantiate wasi
...
void DynamicFunctionRunner::run_program(wasmtime_linker_t *linker_program){
    // Run it.
    wasm_func_t* func;
    wasm_name_t empty;
    wasm_trap_t *trap = NULL;
...
void DynamicFunctionRunner::run_wasm_block(const ndn::Block& block) {

    wasm_module_t* module = compile(block);
...
void DynamicFunctionRunner::setGetBlockFunc(std::function<ndn::optional<ndn::Block>(ndn::Block)> getBlock){
    m_getBlock = getBlock;
}
void DynamicFunctionRunner::setSetBlockFunc(std::function<int(ndn::Block, ndn::Block)> setBlock){
    m_setBlock = setBlock;
}

int DynamicFunctionRunner::getBlockCallback(int len, wasm_memory_t *memory) {
...
int DynamicFunctionRunner::setBlockCallback(int len_param, int len_value, wasm_memory_t *memory) {
    auto mem_arr = wasm_memory_data(memory);
